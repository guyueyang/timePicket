...
public class WheelMain {
	...
	public void initDateTimePicker(int year, int month, int day, int h, int m, int s) {
		...
		// 年
		if (year != -1) {
			wv_year.setAdapter(new NumericWheelAdapter(START_YEAR+"年", END_YEAR+"年"));// 设置"年"的显示数据
...
//			wv_year.setLabel("年");// 添加文字
			...
			OnWheelChangedListener wheelListener_year = new OnWheelChangedListener() {
				public void onChanged(WheelView wheel, int oldValue, int newValue) {
					...
					if (list_big.contains(String.valueOf(wv_month.getCurrentItem() + 1))) {
						wv_day.setAdapter(new NumericWheelAdapter(1+"日", 31+"日"));
					} else if (list_little.contains(String.valueOf(wv_month.getCurrentItem() + 1))) {
						wv_day.setAdapter(new NumericWheelAdapter(1+"日", 30+"日"));
					} else {
						if ((year_num % 4 == 0 && year_num % 100 != 0) || year_num % 400 == 0)
							wv_day.setAdapter(new NumericWheelAdapter(1+"日", 29+"日"));
						else
							wv_day.setAdapter(new NumericWheelAdapter(1+"日", 28+"日"));
					}
				}
			};
		...
		// 月
		if (month != -1) {
			wv_month.setAdapter(new NumericWheelAdapter(1+"月", 12+"月"));
